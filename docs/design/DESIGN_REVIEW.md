...
続いて第3回修正後のdocsを対象に第2周の確認を独立2名で実施し、重複統合後でhigh7件・medium約10件・low8件前後の指摘を得た。下記「第3回・第2周の指摘と修正」の通りdocsへ反映済み。あわせて、確定batch①〜⑤・チャンクPUT・退会batchのSQL固定文列を**docs本文から直接抽出し、node:sqlite(Node v24.19)でDESIGN.mdのスキーマ表から起こしたDDLに対して実行する検算**を行い、37項目すべて合格した(下記「SQL検算」)。

第3周(2026-09-24、収束確認)として、上記の修正が反映済みのdocsを対象に「新規指摘の深掘りではなく、直近ラウンドの修正がdocs間で矛盾なく揃っているか、実装をブロックするhighが残っていないか」を独立に確認した。resume/opponentBoard、観戦チケット廃止(role="spectator"は400)とfinishedへの404、確定batch⑤のCOALESCE、退会batchの順序、Cron2本の式、チャンク転送の10件/alarm制限をピンポイントで照合した結果、**high 0件**。medium 1件(切断・ゲストCookie失効時に結果画面へ抜ける導線が未記載。実装issueのDoDで補う)、low 4件(実装配置表の記載漏れ・表現の不統一など、実装を妨げない)。

以上により、**設計チェックは合格として扱い、issueドラフト作成・実装ループへ進む。** medium 1件はUC-004実装issueのDoDに、low 4件は気づいた担当がついでに直す注記として残す(docs自体の再修正は行わない)。
...
| 重要度 | 指摘 | 対応 |
|---|---|---|
| medium | 切断側・ゲストCookie失効時に結果画面へ抜ける導線が未記載(チケット発行が404 `ROOM_NOT_FOUND`/401 `GUEST_SESSION_EXPIRED`のとき、receiptがあればResult画面へ進む、等) | docsは未修正。UC-004実装issueのDoDに追記して実装時に補う |
| low | account.mdの実装配置表に②-a/②-bの行が無い | 実装時に気づいた担当が追記 |
| low | DO録画削除のタイミングの表現がDESIGN.md(「転送成功」)とbattle.md(「POSTの200受領/410」)でずれる | 実装時に気づいた担当が表現を揃える |
| low | 観戦チケット廃止後、401「role不一致」条件がほぼ使われない | 実装時に不要なら削除 |
| low | チャンクPUTが410で打ち切られた後の最終POSTのchunkCountの値が未定義(どの値でも②のstaging条件で0行になり結果は変わらない) | 実装時に固定値(例: 実送信数)を決めて明記 |

## 次工程の判断
dev-specフェーズ8の「2周しても残るhighは人間に提示して判断を仰ぐ」に従い、第2回high3件・第3回high10件・同第2周high7件をそれぞれ修正し反映した。第3周(収束確認)で独立に照合した結果**high 0件**となり、**設計チェックは合格として扱う**。残るmedium 1件・low 4件は実装をブロックしないため、docsの再修正は行わずissue側のDoD・実装時の補記に委ねる。GitHub issue作成・コミット・push・本体実装はこれから着手する。
