...
<!-- 変更履歴 [2026-09-19]: 第3回レビュー指摘を本文へ反映。補正節を廃し変更履歴化。スキーマ表・インデックス・KV値スキーマ・Cron・期限用語・チケットrole/エラー一覧を確定。 -->
# VS TETRIS 設計書
...
| ランダムマッチの入室期限 | BattleRoom DO初期化から15秒 |
| 退会 | 猶予期間なしで削除開始。外部サービス障害時は削除中を表示し、完了と偽らない |

ルームの待機期限10分・再接続猶予15秒は既存の機能設計値であり、上記マッチング待機30秒とは別の値である。同じ「15秒」でも意味の異なる3つの期限があるため、横断規約「期限・猶予の用語」で用語を固定する。
...
graph TB
    ...
    subgraph Storage["永続化"]
        D1["Cloudflare D1<br/>(Drizzle ORM)<br/>users / account_settings / battle_results / replays<br/>replay_chunks / replay_uploads / reports / account_deletions"]
...
8. 型/lint・フロント・Worker・build・E2Eを実行し、上記コマンドを本体用に確定する。これまでは並列実装の環境が完成したと扱わない。
9. worktree用の除外設定と必要ファイルの供給方法を用意する。プロセス停止は起動したPID／子プロセスだけに限定し、workerd全体を停止しない。
...
| POST | `/api/ws-tickets` | WebSocket接続用の短命チケット発行(横断規約「WebSocket認証(チケット方式)」参照) | 任意(登録済みならJWT検証、無ければゲストCookie) | UC-003, UC-004, UC-005, UC-006 |
...
| GET | `/api/rooms/:roomId/result` | 終了通知の回復 | resultReceipt | UC-004, UC-005 |
...
- **期限・猶予の用語**: 同じ「15秒」が3か所に出るため、全docsで次の用語に統一する。
  ...
  | 用語 | 値 | 意味 |
  |---|---|---|
  | `enterDeadline` | random 15秒 / private 10分 | BattleRoom DO初期化(private は作成)から`waiting`の初回入室を受け付ける期限。接続で延長しない |
  | `disconnectDeadline` | 15秒 | `in_progress`中に切断を検知してから同一`playerRef.id`の再接続を待つ猶予。超過でforfeit_timeout/draw_timeout |
  | `heartbeatTimeout` | 15秒 | アプリheartbeat(5秒ごとのping/pong)の最終受信から無通信と判定するまでの時間。判定後に`disconnectDeadline`を開始する |

  マッチメイキング待機の30秒はこのいずれとも別の値で、UI側の「待機継続／キャンセル」提示タイミングである。
...
- **データ保持期間**:
  ...
  | 対象 | 保持期間 |
  |---|---|
  | 対戦履歴(`battle_results`) | 無期限。ただし退会時の本人データ削除を優先する |
  | リプレイ(`replays` / `replay_chunks` / `replay_uploads`) | 対戦終了時刻(`endedAt`)＋**30日**(既定値)。`discarded`の記録も再生成防止のため同じ期限まで残す |
  | 通報(`reports`) | **MVPでは無期限保持とし、退会時にも削除しない**(`reporter_label`/`target_label`は自己申告名のみで確実な本人照合ができないため。公開前に再確定する。「未解決の論点」参照) |
  | 退会 | 即時削除(猶予期間なし、既定値)。D1の`users`行に加えCognito側のユーザーもAdminDeleteUserで削除する |
  | DOの結果回復用サマリー・receipt hash | 対戦終了後24時間 |
  | DO内の録画データ | 転送成功／破棄時に削除。未成功でも`endedAt`＋30日で削除する |
...
- `reports`の保持期間は「MVPでは無期限保持・退会時も削除しない」を暫定の実装方針とする(横断規約「データ保持期間」)。これは上記の「公開前に確定する」対象そのものであり、確定していない点は変わらない
- `POST /api/reports`のレート制限をCloudflare側のrate limiting rule(インフラ設定)で行う方針は決めたが、具体的な閾値(単位時間あたりの上限・キー)は未確定。セットアップ〜リリース前の運用設定で確定する(docs/design/features/report.md参照)
- リプレイのチャンク数上限40・250ms間引きは記録量の上限を設けるための既定値であり、実測前の見積もりである。結合試験で実際の1試合あたりのイベント量を測って見直す
...
### 設計レビューによる変更履歴
本文を上書きする補正節は置かない。過去のレビューで決めた事項は本文へ畳み込み済みで、ここでは出所だけを記録する。
