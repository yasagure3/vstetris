...
- エラー: `reason`/`reporterLabel`/`targetLabel`欠落 → 400 `REASON_REQUIRED`/`REPORTER_LABEL_REQUIRED`/`TARGET_LABEL_REQUIRED`。`detail`が500文字超 → 400 `DETAIL_TOO_LONG`

### GET /api/reports
- 認証: 必須+運営者権限(DESIGN.md横断規約「運営者権限」参照)
...
| 運営者権限判定(Cognitoグループクレーム読み取り) | domain | `src/server/modules/report/domain/isAdmin.ts`(DESIGN.md「横断規約」を実装、他機能から再利用可能な形にする) |
...
- **上限・境界値**: `detail`は最大500文字(超過は400 `DETAIL_TOO_LONG`)。一覧は20件ずつカーソルページネーション
...
- 単体: `isAdmin`判定(Cognitoグループクレームの有無・複数グループ混在時)
