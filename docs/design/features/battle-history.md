...
- レスポンス(200): `{ "battleResultId": string, "log": <操作ログJSON>, "createdAt": string }`(`battleResultId`は`battle_results.id`。`replays.id`自体はクライアントに公開しない)
- エラー: 対象が存在しない/他人のもの → 404 `REPLAY_NOT_FOUND`。`replays`テーブルの期限切れ削除により本体が既に無い場合も同じ404
...
- **上限・境界値**: 一覧は20件ずつカーソルページネーション。リプレイ保存期間30日(DESIGN.md横断規約)を過ぎたものは本体データ(`replays.log`)を削除するが、`battle_results`行(サマリー)自体は無期限保持のため一覧には残り続け、選択時にのみ404となる
